...
public class PayOrderActivity extends BaseActivity<PayOrderPresenter> implements PayOrderView {
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        if (getIs_group == 1) {//拼团
                            if (getIsGroupEnd.equals("1")) {//最后一位参团
                                ...
                                GroupOrderDetailsActivity.startSelf(PayOrderActivity.this, getOrder_id);
                                finish();
                            } else {
                                OpenGroupSuccessActivity.startSelf(PayOrderActivity.this, getOrder_id);
                                CacheActivity.finishSingleActivityByClass(ConfirmOrderActivity_1.class);
                                finish();
                            }
                        } else {
    ...
    public void getPostSubmitOrderSuccess(int code, WeCatPayBean data) {
        ...
        if (data.getIs_group() == 1) {//拼团商品
            ConstantData.CHANNEL = "3";
            ConstantData.ORDER_ID = getOrder_id;
            ConstantData.GROUP_END = data.getIs_group_end();
        } else {//普通商品
...
//                }
                ...
                getIs_group = data.getIs_group();
                getOrder_id = data.getOrder_id();
                getIsGroupEnd = data.getIs_group_end();
